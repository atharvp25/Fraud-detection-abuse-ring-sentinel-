...
**Production model: XGBoost (Tabular).** It has the best precision/recall/FP-cost 
tradeoff of all five. Isolation Forest and the Ensemble are shown for comparison 
only — their false-positive cost (₹19-20L) makes them unsuitable for production 
use here. Rules Baseline is our "dumb" reference point to show ML actually adds value.


Risk scores below are generated by the **XGBoost (Tabular)** model's predicted 
probability — the same model shown as production-recommended above.
