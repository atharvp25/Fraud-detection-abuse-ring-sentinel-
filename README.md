...
> **Synthetic benchmark performance is not equivalent to production fraud performance.**

This project uses synthetic data generated by known rules. While the data is realistic (7 fraud archetypes, hard negatives from legitimate sharing groups, Gaussian noise injection), the model's near-perfect metrics (F1=0.996, Recall=100%) reflect the synthetic training environment, not real-world conditions. In production deployment:
- Recall would likely be 85–95% due to unseen fraud patterns.
- Continuous retraining on real data would be essential.
- The action-policy thresholds (ALLOW/STEP_UP_AUTH/REVIEW/HARD_BLOCK) would require calibration with domain experts.
